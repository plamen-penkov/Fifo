class fifo_wr_monitor extends uvm_monitor;
	...
	uvm_analysis_port #(fifo_wr_transaction_item) wr_mon_ap;
	...
	virtual function void connect_phase(uvm_phase phase);
		...
		`uvm_info (get_name(), $sformatf("Hello from write monitor connect phase!"), UVM_HIGH)
	...
	virtual task run_phase(uvm_phase phase);
		...
		forever begin
			...
			if (vif.we) begin
				...
				wr_mon_ap.write(tr);
				`uvm_info(get_name(), $sformatf("\ntr.wr_en: %d\ntr.wrdata: %h\ntr.full: %d", tr.wr_en, tr.wrdata, tr.full), UVM_HIGH)
			end
		end
	endtask: run_phase
